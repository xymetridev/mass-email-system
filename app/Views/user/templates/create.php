...
<div class="page-header d-print-none mb-4">
    <div class="row align-items-center">
        <div class="col">
            <h2 class="page-title">Buat Template Pribadi</h2>
            <div class="text-muted mt-1">Desain template email Anda sendiri dan simpan ke pustaka pribadi.</div>
        </div>
        <div class="col-auto ms-auto">
            ...
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal-save-template">
                <i class="ti ti-device-floppy me-2"></i> Simpan ke Pustaka
            </button>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content shadow-lg">
            <div class="modal-header">
                <h5 class="modal-title">Simpan ke Pustaka Pribadi</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <label class="form-label required">Nama Template</label>
                    <input type="text" id="template_name" class="form-control" placeholder="Contoh: Desain Promo Mingguan">
                    <small class="text-muted">Nama ini akan memudahkan Anda mencari template nantinya.</small>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-link link-secondary me-auto" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-primary" id="btn-do-save" onclick="saveTemplate()">
                    <i class="ti ti-check me-2"></i> Simpan Sekarang
                </button>
            </div>
        </div>
    </div>
    ...
    function saveTemplate() {
        ...
        .then(res => {
            if(res.status === 'success') {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="ti ti-check me-2"></i> Simpan Sekarang';
            }
        })
        .catch(err => {
            console.error(err);
